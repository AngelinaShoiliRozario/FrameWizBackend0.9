...
const postLocation = async (req, res) => {
  ...
  if (updated) {
    res.redirect(`/auth/dashboard/${rec._id}`);
  }
  // else {
  //     console.log('Record is not Successfully Updated..............');
...
const dashboard = async (req, res) => {
  ...
  res.redirect(`http://localhost/pagebuilderztrios/views/pages.html?u=${user_id}`);
};
...
const postLogin = async (req, res) => {
  ...
  try {
    ...
    if (this_user) {
      ...
      if (isPasswordValid) {
        res.redirect(`/auth/dashboard/${this_user._id}`);
      } else {
...
const postResetPin = async (req, res) => {
  try {
    ...
    if (updated_user) {
      res.redirect(`/auth/dashboard/${user._id}`);
    } else {
...
const resend = async (req, res) => {
  ...
  request(options, function (error, response) {
    if (error) throw new Error(error);
    res.redirect(`/auth/further_otp/${record._id}`);
  });
};
